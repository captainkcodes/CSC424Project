var todo = {
    ...
    // todo.list() : update todo list HTML
      ...
      // Clear the old list
      var container = document.getElementById("worktodo-list");
      container.innerHTML = "";
    ...
    // todo.add() : add a new item
      ...
      todo.data.push([
        document.getElementById("worktodo-add").value, 0
      ]);
      document.getElementById("worktodo-add").value = "";
      todo.save();
    },
  ...
  window.addEventListener("load", function () {
    document.getElementById("worktodo-dat").addEventListener("click", function () {
      todo.del(0);
    });
    document.getElementById("worktodo-dct").addEventListener("click", function () {
      todo.del(1);
    });
    document.getElementById("worktodo-form").addEventListener("submit", function (evt) {
      evt.preventDefault();
      todo.add();
    });
    todo.load();
